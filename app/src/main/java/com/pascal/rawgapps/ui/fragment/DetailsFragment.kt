...
import com.pascal.rawgapps.databinding.FragmentDetailsBinding
import com.pascal.rawgapps.viewmodel.DetailsFragmentViewModel
...
class DetailsFragment : Fragment() {
    ...
    private fun observeLiveData() {
        viewModel.gameDetails.observe(viewLifecycleOwner, Observer {
            it?.let {
                ...
                it.releaseDate?.let { releasedate ->
                    ...
                    binding.TextViewFragmentDetailsReleaseDate.visibility =
                        View.GONE; binding.textView12.visibility = View.GONE
                }


                Glide.with(this).load(it.backgroundImage)
                    .into(binding.ImageViewFragmentDetailsImage)
        ...
        viewModel.isSavedOnDB.observe(viewLifecycleOwner, Observer {
            it?.let {
                if (it) {
                    binding.ImageViewFragmentDetailsWishlist.setImageDrawable(
                        context!!.getDrawable(
                            R.drawable.ic_baseline_card_giftcard_24_green
                        )
                    )
                } else {
                    binding.ImageViewFragmentDetailsWishlist.setImageDrawable(
                ...
                viewModel.showAlertDialog(
                    ...
                    positiveOnclick = {
                        Toast.makeText(
                            ...
                            Toast.LENGTH_SHORT
                        ).show();viewModel.deleteFromWishlist(context!!)
                    },
